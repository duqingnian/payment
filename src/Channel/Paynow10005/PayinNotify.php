...
class PayinNotify extends \App\Channel\BasePayinNotify
{
	public function get_message()
	{
		...
		{
			$this->e('merchantOrderNo is missing');
		}
		
		$order = $this->entityManager->getRepository(\App\Entity\OrderPayin::class)->findOneBy(['pno'=>$data['merchantOrderNo']]);
		if(!$order)
		{
			$this->e('order:'.$data['merchantOrderNo'].' not found');
		}
		...
		$return['real_amount'] = $data['payAmount'];
