...
class PayinNotify extends \App\Channel\BasePayinNotify
{
	public function get_message()
	{
		...
		{
			$this->e('out_trade_no is missing');
		}
		
		$order = $this->entityManager->getRepository(\App\Entity\OrderPayin::class)->findOneBy(['pno'=>$data['out_trade_no']]);
		if(!$order)
		{
			$this->e('order:'.$data['out_trade_no'].' not found');
		}
		...
		if(abs($data['amount'] - $data['amount_true']) > 1)
		{
			$real_amount = $data['amount_true'];
		}
