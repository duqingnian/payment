...
class PayoutNotify extends \App\Channel\BasePayoutNotify
{
	public function get_message()
	{
		...
		if(!array_key_exists('merOrderNo',$data))
		{
			$this->e('merOrderNo IS MISSING');
		}

		$order = $this->entityManager->getRepository(\App\Entity\OrderPayout::class)->findOneBy(['pno'=>$data['merOrderNo']]);
		if(!$order)
		{
			$this->e('order:'.$data['merOrderNo'].' not found!');
		}
