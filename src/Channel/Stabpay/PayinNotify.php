...
class PayinNotify extends \App\Channel\BasePayinNotify
{
	public function get_message()
	{
		...
		{
			$this->e('orderNo is missing');
		}
		
		$order = $this->entityManager->getRepository(\App\Entity\OrderPayin::class)->findOneBy(['pno'=>$data['orderNo']]);
		if(!$order)
		{
			$this->e('order:'.$data['orderNo'].' not found');
		}
