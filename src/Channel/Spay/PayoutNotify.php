...
class PayoutNotify extends \App\Channel\BasePayoutNotify
{
	public function get_message()
	{
		...
		if(!array_key_exists('merchant_orderno',$data))
		{
			$this->e('merchant_orderno IS MISSING');
		}

		$order = $this->entityManager->getRepository(\App\Entity\OrderPayout::class)->findOneBy(['pno'=>$data['merchant_orderno']]);
		if(!$order)
		{
			$this->e('order: '.$data['merchant_orderno'].' not found!');
		}
		...
		$return['channel_order_no'] = $data['orderno'];
