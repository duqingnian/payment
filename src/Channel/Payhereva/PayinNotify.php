...
namespace App\Channel\Payhereva;
...
class PayinNotify extends \App\Channel\BasePayinNotify
{
	public function get_message()
	{
		...
		$channel_pay_method_setting = $this->entityManager->getRepository(\App\Entity\ChannelPaymentMethodSetting::class)->findOneBy(['mid'=>$order->getMid(),'cid'=>$order->getCid(),'method'=>'virtual_account']);
		if($channel_pay_method_setting)
		{
			//查找支付方式关联通道，更新通道费率和金额
			$channel_pay_method = $this->entityManager->getRepository(\App\Entity\ChannelPayMethod::class)->findOneBy(['cid'=>$order->getCid(),'method'=>'virtual_account']);
